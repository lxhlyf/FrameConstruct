...
//                    getParent().requestDisallowInterceptTouchEvent(false)
            if (Math.abs(deltaX) > Math.abs(deltaY)) {
                ...
                if (getCurrentItem() == 0 && deltaX > 0) {
                    ...
                    Log.i("BannerViewPager", "Down-请求父类拦截");

                } else if (getCurrentItem() == (mAdapter.getCount() - 1) && deltaX < 0) {

                    setCurrentItem(0);
                } else {
                    springMenu.addIgnoredView(this);
                    getParent().requestDisallowInterceptTouchEvent(true);
                    Log.i("BannerViewPager", "Down-请求父类不要拦截");
                }
            } else {
                //竖直方向滑动
                getParent().requestDisallowInterceptTouchEvent(false);
        ...
        return super.dispatchTouchEvent(ev);  //返回false 相当于请求父类拦截， true //子类拦截
    }*/
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        //TODO:连点俩下就会崩了， An operation is not implemented: not implemented
